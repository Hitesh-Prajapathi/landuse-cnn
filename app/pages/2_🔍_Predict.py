...
if uploaded_file is not None:
    ...
    # ── API Health Check ───────────────────────────────────────────────
    # Before sending the image, confirm the FastAPI backend is up and the
    # model is loaded. If not, show a helpful error with startup instructions.
    try:
    ...
    if not api_ok:
        st.error(
            "🔴 **API is not available.** Please start the FastAPI backend first:\n\n"
            "```\nuvicorn app.api:app --host 0.0.0.0 --port 8000\n```"
        )
    else:
        # ── Send Image to FastAPI ─────────────────────────────────────
        # We POST the image as multipart/form-data to /predict.
        # uploaded_file.seek(0) resets the file pointer because PIL.open()
        # consumed it during the preview step above.
        ...
        if response.status_code == 200:
            ...
            with col_chart:
                ...
                # Plotly horizontal bar chart — works better than st.bar_chart
                # for our use case because we need custom colours and labels.
                fig = go.Figure(go.Bar(
                    x=probs,
                    y=classes,
                    orientation='h',
                    marker_color=colors,
                    text=[f"{p:.1f}%" for p in probs],  # Label on each bar
                    textposition='outside',
                    textfont=dict(color='#e2e8f0', size=12),
                ))

                fig.update_layout(
                    plot_bgcolor='rgba(0,0,0,0)',        # Transparent (matches dark theme)
                    paper_bgcolor='rgba(0,0,0,0)',
                    font=dict(color='#a0aec0', family='Inter'),
                    xaxis=dict(
                        title="Confidence (%)",
                        range=[0, max(probs) * 1.25],   # Extra space for text labels
            ...
            with col_info:
                ...
                # ── Top-3 Predictions ─────────────────────────────────
                # Secondary display showing the runner-up predictions with medals.
                st.markdown("")
